...
async function getLatencyAnalysis(dateFrom, dateTo, office = null) {
  ...
  // Filter to confirmed orders only (same exclusion rule as the rest of the
  // dashboard), and to this office if scoped.
  ...
  // Batch-resolve the linked Quote's "Quoted" (quote date) by id. Same
  // pattern as the Account/Customer name resolution in salesPerformance.js -
  // the embedded lookup name/fields aren't reliably returned for this app's
  // OAuth client, so we resolve directly from the Quotes module instead.
  ...
  let quoteToOrderAnomalies = 0; // negative latency (quote dated after PO)
  ...
  return {
    ...
    dataQuality: {
      ...
      quoteToOrderAnomalies,   // linked quote dated AFTER the order's PO_Date
      orderToShipAnomalies     // shipped before the order's own PO_Date (data entry issue)
    }
  };
}
