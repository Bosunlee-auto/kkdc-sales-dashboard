...
async function getLatencyAnalysis(dateFrom, dateTo, office = null) {
  ...
  for (let i = 0; i < quoteIds.length; i += CHUNK) {
    ...
    idChunkPromises.push(coqlQuery(`SELECT id, Quoted, QREF FROM Quotes WHERE id in (${idList})`));
  }
  const idChunkResults = await Promise.all(idChunkPromises);
  for (const rows of idChunkResults) {
    for (const q of rows) {
      quoteInfoById[q.id] = { quoted: q.Quoted, qref: q.QREF };
    }
  }

  // First-Quote-per-lineage: find the EARLIEST Quoted date across every
  // revision sharing each QREF (not just the specific revision linked to
  // the order), so "First Quote -> Order" reflects true first-contact date
  // even if the order was placed against a later revision.
  ...
  for (let i = 0; i < qrefs.length; i += CHUNK) {
    ...
    qrefChunkPromises.push(coqlQuery(`SELECT QREF, Quoted FROM Quotes WHERE QREF in (${qrefList})`));
  }
  const qrefChunkResults = await Promise.all(qrefChunkPromises);
  for (const rows of qrefChunkResults) {
    for (const r of rows) {
      if (!r.Quoted || !r.QREF) continue;
      const existing = firstQuoteDateByQref[r.QREF];
      if (!existing || r.Quoted < existing) firstQuoteDateByQref[r.QREF] = r.Quoted;
    }
  }
  ...
  for (const row of relevantRows) {
    ...
    if (!info || !info.quoted) {
      ...
      const linkedQ2O = daysBetween(info.quoted, row.PO_Date);
  ...
  const poRows = await coqlQuery(
    `SELECT id, PO_Date, OUS_Ref, Territory_Div FROM Purchase_Orders
     WHERE (PO_Date >= '${dateFrom}') AND (PO_Date <= '${dateTo}')`
  );
