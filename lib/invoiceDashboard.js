...
function emptyPeriod() {
  return {
    ...
    netProductValue: 0,     // Net_Product_Value1, subtotal before freight/duty/S&H (all valid = Shipped+Scheduled+Forecast)
    netProductValueNY: 0,
    netProductValueNJ: 0,
    netProductValueShipped: 0,
    netProductValueShippedNY: 0,
    netProductValueShippedNJ: 0,
    netProductValueScheduled: 0,
    netProductValueScheduledNY: 0,
    netProductValueScheduledNJ: 0,
    netProductValueForecast: 0,
    netProductValueForecastNY: 0,
    netProductValueForecastNJ: 0,
...
async function getInvoiceDashboard(dateFrom, dateTo, granularity = 'month') {
  ...
  //
  // BUG FIX (2026-09-05, same root cause found in lib/salesPerformance.js):
  // that extension is only valid when dateTo's year is the real, current
  // calendar year - a genuine "forecast the rest of this year" case.
  // Applied unconditionally, it would silently stretch a past year's cutoff
  // to that year's Dec 31 even when dateTo was deliberately truncated
  // earlier (e.g. the "prior" side of a future YoY comparison against an
  // in-progress current year), pulling in real actuals from after the
  // matched cutoff and inflating the prior-year total. No YoY/prior-year
  // caller exists for this dashboard yet, so this is a no-op today - fixed
  // now so a future Total Invoice YoY feature doesn't inherit the same bug.
  ...
  for (const row of invRows) {
    ...
    if (isShipped) {
      ...
      b.netProductValueShipped += netProductValue;
      if (office === 'NY') b.netProductValueShippedNY += netProductValue;
      else if (office === 'NJ') b.netProductValueShippedNJ += netProductValue;
    } else {
      ...
      if (row.SHIP_DATE) {
        ...
        b.netProductValueScheduled += netProductValue;
        if (office === 'NY') b.netProductValueScheduledNY += netProductValue;
        else if (office === 'NJ') b.netProductValueScheduledNJ += netProductValue;
      } else {
        ...
        b.netProductValueForecast += netProductValue;
        if (office === 'NY') b.netProductValueForecastNY += netProductValue;
        else if (office === 'NJ') b.netProductValueForecastNJ += netProductValue;
      }
    }
  ...
  // --- Forecast (#4) - independent live query ------------------------------
  // BUG FOUND 2026-08-25 (Bosun): Forecast was always showing $0. Root
  // cause: the main invRows query above starts with
  // "WHERE (SHIP_DATE >= dateFrom) AND (SHIP_DATE <= esdCutoff)" - a
  // Ship_Date comparison against NULL never matches, so any invoice with NO
  // Ship_Date (exactly what defines "Forecast") was structurally excluded
  // from ever being fetched at all. Not a business finding of "no forecast
  // pipeline exists" - a query-scoping bug. Fixed the same way as Overdue:
  // an independent query, floored using a rolling window (see
  // FORECAST_FLOOR_DATE below) to exclude old legacy invoices that simply
  // predate consistent Ship_Date usage (confirmed live: 200+ null-Ship_Date
  // invoices exist going back to 2015-2019, virtually all long since
  // resolved). Bucketed into today's period, same "right now" snapshot
  // logic as Overdue.
  // FORECAST_FLOOR_DATE is a ROLLING window - 3 months before today, not a
  // fixed date. Per Bosun (2026-08-25): a not-yet-shipped invoice with no
  // Ship_Date that's older than 3 months is effectively dead/cancelled in
  // practice, even if never formally marked so in CRM - it should NOT be
  // counted as genuine forecast pipeline. Recomputed on every call so the
  // window always reflects "as of today," not a snapshot in time.
  const threeMonthsAgo = new Date();
  threeMonthsAgo.setUTCMonth(threeMonthsAgo.getUTCMonth() - 3);
  const FORECAST_FLOOR_DATE = threeMonthsAgo.toISOString().slice(0, 10);
  const forecastRows = await coqlQuery(
    `SELECT Invoice_No, Amount, Net_Product_Value1, Office_Div, Sales_Order FROM Invoices ` +
      `WHERE (SHIP_DATE is null AND Invoice_Date >= '${FORECAST_FLOOR_DATE}')`
  );
  if (forecastRows.length > 0) {
    const forecastSoIds = [...new Set(forecastRows.map((r) => r.Sales_Order && r.Sales_Order.id).filter(Boolean))];
    const unseenForecastSoIds = forecastSoIds.filter((id) => !soDataById[id]);
    for (let i = 0; i < unseenForecastSoIds.length; i += CHUNK) {
      const chunkIds = unseenForecastSoIds.slice(i, i + CHUNK);
      const idList = chunkIds.map((id) => `'${id}'`).join(',');
      const soRows = await coqlQuery(`SELECT id, Status FROM Sales_Orders WHERE id in (${idList})`);
      for (const so of soRows) {
        soDataById[so.id] = soDataById[so.id] || { status: so.Status, cost: 0, freight: 0 };
    ...
    const todayKey = periodKey(new Date().toISOString().slice(0, 10), granularity);
    const forecastBucket = bucket(todayKey);
    for (const row of forecastRows) {
      const soId = row.Sales_Order && row.Sales_Order.id;
      const soInfo = soId ? soDataById[soId] : null;
      // Exclude Superceded/On Hold/Cancelled, AND anything already Shipped
      // (a Shipped order should always have a Ship_Date - if one somehow
      // doesn't, it belongs in the Shipped bucket's data-quality concerns,
      // not counted twice here as Forecast).
      if (soInfo && (EXCLUDED_SO_STATUSES.includes(soInfo.status) || soInfo.status === 'Shipped')) continue;

      const amount = Number(row.Amount || 0);
      const netProductValue = Number(row.Net_Product_Value1 || 0);
      const office = normalizeOffice(row.Office_Div);
      forecastBucket.invoiceForecastTotal += amount;
      if (office === 'NY') forecastBucket.invoiceForecastTotalNY += amount;
      else if (office === 'NJ') forecastBucket.invoiceForecastTotalNJ += amount;
      forecastBucket.netProductValueForecast += netProductValue;
      if (office === 'NY') forecastBucket.netProductValueForecastNY += netProductValue;
      else if (office === 'NJ') forecastBucket.netProductValueForecastNJ += netProductValue;
      // Also roll into the flat combined totals - the Invoice-vs-NPV line
      // chart reads these directly and would otherwise silently exclude
      // Forecast, same bug as the KPI tables had before this fix.
      forecastBucket.invoiceTotal += amount;
      if (office === 'NY') forecastBucket.invoiceTotalNY += amount;
      else if (office === 'NJ') forecastBucket.invoiceTotalNJ += amount;
      forecastBucket.netProductValue += netProductValue;
      if (office === 'NY') forecastBucket.netProductValueNY += netProductValue;
      else if (office === 'NJ') forecastBucket.netProductValueNJ += netProductValue;
      forecastBucket.invoiceCount += 1;
    }
  }
