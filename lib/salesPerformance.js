...
async function getSalesPerformance(dateFrom, dateTo, granularity = 'month', office = null) {
  ...
  if (pendingEsdRows.length > 0) {
    ...
    for (const pending of pendingEsdRows) {
      ...
      // Fall back to today's period if no vendor PO / ETD is on file yet -
      // better to show it as "due now" than to silently drop it.
      const key = etd ? periodKey(etd, granularity) : periodKey(new Date().toISOString().slice(0, 10), granularity);
  ...
  const shippedRows = await coqlQuery(
    `SELECT Quoted_Price, Status, NY_Office_Full_Credit, NJ_Office_Full_Credit, Ship_Date
     FROM Sales_Orders
