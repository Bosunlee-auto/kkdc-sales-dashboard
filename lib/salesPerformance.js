...
async function getSalesPerformance(dateFrom, dateTo, granularity = 'month', office = null) {
  ...
  //
  // Upper bound is Dec 31 of dateTo's year, NOT dateTo itself: sales
  // performance is tracked annually, and forthcoming ESD should show for
  // every remaining month of the CURRENT year (this month through Dec 31),
  // not just up to "today" (which is what dateTo is, for an in-progress
  // current-year query). Nothing can actually have SHIPPED in the future,
  // so this extension only ever adds pending/forecasted rows, never
  // fabricates real shipments. For a past-year query (dateTo already =
  // that year's Dec 31), this extension is a no-op.
