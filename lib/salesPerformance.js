...
async function getSalesPerformance(dateFrom, dateTo, granularity = 'month', office = null) {
  ...
  for (const row of pendingSoRows) {
    ...
    if (office === 'NY') {
      amount = nyCredit;
      if (!(quotedPrice > 0 && nyCredit / quotedPrice > 0)) continue;
    } else if (office === 'NJ') {
      amount = njCredit;
      if (!(quotedPrice > 0 && njCredit / quotedPrice > 0)) continue;
    }
  ...
  if (pendingEsdRows.length > 0) {
    ...
    for (const pending of pendingEsdRows) {
      ...
      // Also clamp/skip if the ETD itself lands outside the requested
      // [dateFrom, dateTo] window (e.g. a vendor PO promised for a date
      // beyond what the user asked to see) - otherwise a future ETD can
      // silently add an extra period to the chart's x-axis beyond the
      // selected range. Since this query is no longer PO_Date-bound, this
      // clamp is now the ONLY thing keeping an old backlog order's ESD out
      // of a query window it doesn't belong in.
      if (effectiveDate < dateFrom || effectiveDate > dateTo) continue;
  ...
  // --- 3b. Shipped / Net Sales - separately queried by Ship_Date ----------
