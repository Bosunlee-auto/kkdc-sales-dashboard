...
async function getSalesPerformance(dateFrom, dateTo, granularity = 'month', office = null) {
  ...
  const soRows = await coqlQuery(
    `SELECT Quoted_Price, Status, NY_Office_Full_Credit, NJ_Office_Full_Credit, PO_Date, Reference_no, GPM1, COST_TTL
     FROM Sales_Orders
  ...
  for (const row of soRows) {
    ...
    // "Ordered to vendor" (confirmed, not yet shipped) still counts toward
    // Shipped+ESD as a forward-looking estimate, bucketed by PO_Date since
    // the CRM's expected-ship-date fields (Vendor_ESD / Branch_ESD) are not
    // queryable via COQL (confirmed live 2026-08-24 - INVALID_QUERY on both).
    // This is a known simplification, not a bug: actual Shipped amounts
    // (below) are always Ship_Date-accurate; only this forward-looking
    // portion uses the less-precise PO_Date placeholder.
