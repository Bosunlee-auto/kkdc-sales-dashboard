...
async function getSalesPerformance(dateFrom, dateTo, granularity = 'month', office = null) {
  ...
  const pendingEsdRows = []; // "Ordered to vendor" rows awaiting ETD-based bucketing (see Part 3a below)
  ...
  const soRows = await coqlQuery(
    `SELECT id, Quoted_Price, Status, NY_Office_Full_Credit, NJ_Office_Full_Credit, PO_Date, Reference_no, GPM1, COST_TTL
     FROM Sales_Orders
  ...
  // --- 3a. Shipped+ESD forward-looking portion - "Ordered to vendor" ------
  // orders, queried INDEPENDENTLY of PO_Date -------------------------------
  //
  // BUG FOUND 2026-08-25 (Bosun cross-check): the original version only
  // looked at "Ordered to vendor" rows already present in the PO_Date-
  // filtered soRows above - but an order placed WELL BEFORE the requested
  // window (even back in 2017) can still be sitting in "Ordered to vendor"
  // today, waiting to ship. Verified live: for NY, $355,782 total is
  // currently "Ordered to vendor," but $104,525 of that (15 orders) has a
  // PO_Date before 2026 and was therefore silently invisible to a 2026-only
  // query - understating the forward-looking ESD figure by ~30%. This is
  // the same class of bug as the earlier Shipped/PO_Date fix, recurring
  // here: an order's CURRENT status can be relevant to a period far removed
  // from when it was originally placed. Fixed by querying "Ordered to
  // vendor" status directly, with NO PO_Date filter at all - it's an
  // inherently small, bounded backlog (only active not-yet-shipped orders),
  // safe to fetch in full regardless of the requested date window.
  const pendingSoRows = await coqlQuery(
    `SELECT id, Quoted_Price, NY_Office_Full_Credit, NJ_Office_Full_Credit FROM Sales_Orders WHERE Status = '${BUCKET.ORDERED_TO_VENDOR}'`
  );
  const seenPendingIds = new Set();
  for (const row of pendingSoRows) {
    if (row.id) {
      if (seenPendingIds.has(row.id)) continue;
    ...
    let amount = quotedPrice;
    let esdOfficeShare = 1;
    if (office === 'NY') {
      amount = nyCredit;
      esdOfficeShare = quotedPrice > 0 ? nyCredit / quotedPrice : 0;
      if (esdOfficeShare === 0) continue; // not an NY order at all - matches the main loop's exact skip condition
    } else if (office === 'NJ') {
      amount = njCredit;
      esdOfficeShare = quotedPrice > 0 ? njCredit / quotedPrice : 0;
      if (esdOfficeShare === 0) continue; // not an NJ order at all
    }
    pendingEsdRows.push({ id: row.id, amount });
  }

  // --- 3b. Resolve ETD (expected ship date) for pending "Ordered to vendor"
  // orders, via their linked Purchase_Order(s) -----------------------------
  // Per Bosun (2026-08-25): a past month should show $0 ESD (everything
  // that was going to ship, already has); the CURRENT month can show both
  // Shipped and forthcoming ESD; future months should show ONLY the ESD
  // portion (nothing can have shipped yet). That only works if each order's
  // not-yet-shipped amount lands in the month it's ACTUALLY expected to
  // ship, not the month it was ordered. Purchase_Orders.ETD (confirmed live
  // 2026-08-24, queryable via COQL) gives that real date; Sales_Orders'
  // own Vendor_ESD/Branch_ESD fields are NOT queryable (INVALID_QUERY).
  // An order can have multiple vendor POs (multiple vendors/partial
  // shipments) - uses the LATEST ETD among them, since the order isn't
  // fully ready to ship until every vendor's portion has arrived.
  if (pendingEsdRows.length > 0) {
    const soIds = [...new Set(pendingEsdRows.map((r) => r.id))];
    const etdChunkPromises = [];
    for (let i = 0; i < soIds.length; i += 50) {
      const chunk = soIds.slice(i, i + 50);
      const idList = chunk.map((id) => `'${id}'`).join(',');
      etdChunkPromises.push(coqlQuery(`SELECT OUS_Ref, ETD FROM Purchase_Orders WHERE OUS_Ref in (${idList})`));
    }
    const etdChunkResults = await Promise.all(etdChunkPromises);
    const latestEtdBySoId = {};
    for (const rows of etdChunkResults) {
      for (const po of rows) {
        const soId = po.OUS_Ref && po.OUS_Ref.id;
        if (!soId || !po.ETD) continue;
        if (!latestEtdBySoId[soId] || po.ETD > latestEtdBySoId[soId]) latestEtdBySoId[soId] = po.ETD;
      }
    }

    for (const pending of pendingEsdRows) {
      const etd = latestEtdBySoId[pending.id];
      // Fall back to the END of the requested range (dateTo) if no vendor
      // PO / ETD is on file yet - NOT "today". Using "today" was a bug:
      // for a historical-year query (e.g. the Quota Progress year selector
      // showing 2024), "today" is in 2026 and would inject a stray
      // out-of-range bucket into a 2024-only result set.
      const effectiveDate = etd || dateTo;

      // ESD upper bound is Dec 31 of dateTo's year, NOT dateTo itself.
      // BUG FOUND 2026-08-25 (Bosun): sales performance is tracked on an
      // annual basis, and forthcoming ESD should show for every remaining
      // month of the CURRENT year (this month through Dec 31), not just up
      // to "today." The Quota Progress widget queries the current year as
      // dateFrom=Jan 1, dateTo=today (correct for the Shipped side, since
      // nothing can have shipped in the future) - but reusing that same
      // dateTo to clamp ESD was cutting off Oct/Nov/Dec ETDs entirely, so
      // only already-overdue (ETD <= today) pending orders ever showed up,
      // and genuinely upcoming ESD (the whole point of this feature) was
      // silently dropped. dateFrom is left as the lower bound unchanged -
      // for a past-year query (e.g. 2024, dateTo already = 2024-12-31),
      // this upper-bound extension is a no-op.
      const esdCutoff = `${new Date(dateTo).getUTCFullYear()}-12-31`;
      if (effectiveDate < dateFrom || effectiveDate > esdCutoff) continue;

      const key = periodKey(effectiveDate, granularity);
      bucket(key).shippedPlusESDAmount += pending.amount;
    }
  }

  // --- 3c. Shipped / Net Sales - separately queried by Ship_Date ----------
  // See the bug-fix note above. This is intentionally a SEPARATE query from
  // the PO_Date-filtered one above, because an order's PO_Date and Ship_Date
  // can fall in different years/periods and each metric needs its own
  // correct date driver.
  const shippedRows = await coqlQuery(
    `SELECT id, Quoted_Price, Status, NY_Office_Full_Credit, NJ_Office_Full_Credit, Ship_Date
     FROM Sales_Orders
     WHERE (Ship_Date >= '${dateFrom}') AND (Ship_Date <= '${dateTo}')`
  );
  ...
  for (const row of shippedRows) {
    ...
    const displayStatus = toDisplay(row.Status);
    if (displayStatus !== BUCKET.SHIPPED) continue; // Ship_Date can be pre-filled before actual shipment on some records
