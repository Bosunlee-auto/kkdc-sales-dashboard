...
async function getSalesPerformance(dateFrom, dateTo, granularity = 'month') {
  ...
  // record detail as "10. SALES SUMMARY - GPM / GPM %"). GPM1's revenue basis
  // is NOT the same as Quoted_Price (confirmed live: GPM1 + COST_TTL !=
  // Quoted_Price for the same record), so Margin % is computed the same way
  // CRM does it: GPM1 / (GPM1 + COST_TTL) - NOT GPM1 / Quoted_Price.
...
// ---------------------------------------------------------------------------
// Hardened 2026-08-23: live COQL checks confirmed the data itself is fine -
// the dominant "(blank)" bar seen on the dashboard was a chart rendering
// issue (label collision from cramped chart height), not bad data or bad
// aggregation logic. This version adds a defensive dedupe-by-id plus
// trimmed key normalization, but the real fix is the chart height/color
// fix already in public/index.html.
