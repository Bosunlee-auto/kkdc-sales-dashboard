...
async function getSalesPerformance(dateFrom, dateTo, granularity = 'month', office = null) {
  ...
  for (const row of quoteRows) {
    const div = row.Territory_Div;
    const isNY = div === 'NY Territory';
    const isNJ = div === 'NJ Territory';
    // Territory_Div (field_label "Office Div") picklist is confirmed live
    // (2026-09-05) to have FOUR options: '-None-', 'NY Territory',
    // 'NJ Territory', 'Joint Territory' - the code here previously only
    // ever matched the first two, silently excluding every Joint Territory
    // quote from BOTH offices' totals (worse still: for a single-office
    // fetch, the `continue` below used to drop these rows before they were
    // even counted anywhere). Unlike Sales_Orders, Quotes has no stored
    // NY_Office_Full_Credit/NJ_Office_Full_Credit-style formula field that
    // pre-splits Joint Territory - confirmed via ZohoCRM_getFields, no such
    // field exists on this module - so the 50/50 split has to happen here,
    // matching the same convention Sales_Orders already applies.
    const isJoint = div === 'Joint Territory';

    if (office === 'NY' && !isNY && !isJoint) continue;
    if (office === 'NJ' && !isNJ && !isJoint) continue;

    const b = bucket(periodKey(row.Quoted, granularity));
    const amount = Number(row.Amount || 0);
    b.quoteCount += 1;

    if (office === 'NY' || office === 'NJ') {
      // Single-office view: this office's own share only - full amount for
      // its own territory, half for Joint Territory.
      b.quoteAmount += isJoint ? amount / 2 : amount;
    } else {
      // Combined/Total view: the whole company-wide amount once, regardless
      // of territory - never split or duplicated here.
      b.quoteAmount += amount;
    }

    if (isNY) b.quoteAmountNY += amount;
    else if (isNJ) b.quoteAmountNJ += amount;
    else if (isJoint) {
      b.quoteAmountNY += amount / 2;
      b.quoteAmountNJ += amount / 2;
    }
  }
