...
async function getSalesPerformance(dateFrom, dateTo, granularity = 'month', office = null) {
  ...
  for (const row of soRows) {
    ...
    // "Ordered to vendor" (confirmed, not yet shipped) counts toward
    // Shipped+ESD as a forward-looking estimate - but bucketed by the
    // linked vendor Purchase_Order's actual ETD (expected ship date), NOT
    // PO_Date. Collected here and resolved after this loop, once we know
    // every relevant SO id to batch-query Purchase_Orders for.
    if (displayStatus === BUCKET.ORDERED_TO_VENDOR) {
      pendingEsdRows.push({ id: row.id, amount });
    }
  }

  // --- 3a. Resolve ETD (expected ship date) for pending "Ordered to vendor"
